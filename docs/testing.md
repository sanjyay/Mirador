...
* With three workspaces, select the middle card and verify that at least 40% of each neighboring card is visible. Preview width and height should be 10% larger than the previous local version (32% larger than the initial neighbor-focused layout), unless limited by the usable monitor bounds. Workspace borders should be absent while the selected number stays highlighted. Badges overlay the preview; source proportions stay correct throughout the scroll animation.
